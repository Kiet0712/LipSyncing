...
class CropAndExtract():
    def __init__(self, sadtalker_path, device):
        ...
        self.net_recon = networks.define_net_recon(net_recon='resnet50', use_last_fc=False, init_path='SadTalker/').to(device)
    ...
    def generate(self, input_path, save_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256):
        ...
        if not os.path.isfile(coeff_path):
            ...
            for idx in tqdm(range(len(frames_pil)), desc='3DMM Extraction In Video:'):
                ...
                im_t = torch.tensor(np.array(im1)/255., dtype=torch.float16).permute(2, 0, 1).to(self.device).unsqueeze(0)
