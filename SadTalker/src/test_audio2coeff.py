...
class Audio2Coeff():
    ...
    def generate(self, batch, coeff_save_dir, pose_style, ref_pose_coeff_path=None):
        ...
        with torch.no_grad():
            #test
            batch['indiv_mels'] = batch['indiv_mels'].to(torch.float16)
            batch['ref'] = batch['ref'].to(torch.float16)
            batch['ratio_gt'] = batch['ratio_gt'].to(torch.float16)
