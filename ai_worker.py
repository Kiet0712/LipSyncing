...
def process_lipsync_job_callback(ch, method, properties, body):
    ...
    fragmenting the output video, and publishing segments via Redis Pub/Sub.
    """
    session_id = None
    segment_number = None
    temp_audio_path = None
    temp_video_path = None
    temp_fmp4_dir = None # This remains for the TemporaryDirectory context manager
    ...
    try:
        ...
        # Run SadTalker inference to generate the lipsync video segment
        logger.info(f"Running SadTalker inference for session {session_id}, segment {segment_number}...")
        video_output_bytes = run_lipsync_inference_from_bytes(
            image_bytes=image_bytes,
            audio_bytes=open(temp_audio_path, "rb").read(), # Read audio content again
            enhancer=enhancer,
            use_cpu=use_cpu,
            verbose=False # Set to True for SadTalker's verbose output (for debugging)
        )
        logger.info(f"SadTalker inference completed for session {session_id}, segment {segment_number}.")

        # Save SadTalker output to a temporary MP4 file
        with tempfile.NamedTemporaryFile(delete=False, suffix=".mp4") as temp_video_file:
            temp_video_file.write(video_output_bytes)
            temp_video_path = temp_video_file.name
        logger.info(f"Temporary SadTalker output video saved to: {temp_video_path}")
        
        # Create a temporary directory for fMP4 segments and fragment the video
        with tempfile.TemporaryDirectory() as temp_fmp4_dir_ctx:
            temp_fmp4_dir = temp_fmp4_dir_ctx # Assign to outer scope variable for finally block reference
            fmp4_segments = fragment_mp4_to_fmp4(temp_video_path, temp_fmp4_dir)
        ...
        # No need for explicit shutil.rmtree(temp_fmp4_dir) here.
